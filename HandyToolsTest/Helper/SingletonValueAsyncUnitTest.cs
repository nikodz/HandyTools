...
namespace HandyToolsTest.Helper
{
	public class SingletonValueAsyncUnitTest
	{
		...
		{
			var counter = 0;
			var singleValue = new SingletonValueAsync<int>(async () => { counter++; return await Task.FromResult(counter - 1); });
		...
		{
			var counter = 0;
			var singleValue = new SingletonValueAsync<string>(async () => { counter++; return await Task.FromResult("test" + counter); });
			var a = await singleValue;
			var b = await singleValue;
			Assert.Equal(1, counter);
